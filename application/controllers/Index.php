...
class Index extends CI_Controller {
    ...
    function forgetsubmit() {
		...
		$query		= $this->m_global->getByID('email',$email,'user')->row_array();
		$id			= $query['id'];
		$pk  			= md5( $id );
		$link       	= base_url('index/reset').'/'.$id.'/'.$pk;
		
		$contentemail	= 'Berikut ini adalah link lupa password pada kursus brevet Tax Center Universitas Gunadarma. Silahkan klik link berikut ini untuk merubah password anda '.$link;
		
		$this->email->to($email);
		$this->email->from('noreply@taxcenter.gunadarma.ac.id','No Reply Tax Center UG');
		$this->email->Subject('Lupa Password Kursus Brevet');
		$this->email->message($contentemail);
		$this->email->send();
		
		$isi['kelassingle'] = 'Forget';$isi['kelasroot']   = ''; $isi['kelas']     = '';
        $isi['judul']       = 'Forget Password';
        $isi['content']     = 'frontend/forget-password-submit';
        $this->load->view('frontend/index',$isi);
    }
    ...
    function reset() {
		...
		if ($pk){
			$isi['kelassingle'] = 'Forget';$isi['kelasroot']   = ''; $isi['kelas']     = '';
			$isi['judul']       = 'Reset Password';
			$isi['id']       	= $id;
			$isi['pk']       	= $pk;
			$isi['content']     = 'frontend/reset-password';
			$this->load->view('frontend/index',$isi);
		}else{
    ...
    function createregister() {
        if ($this->input->post('submit')){
			...
			$contentemail	= 'Terima Kasih anda telah mendaftar sebagai peserta kursus brevet yang diselenggarakan oleh Tax Center Universitas Gunadarma. Silahkan melakukan pembayaran di ruang D117 Tax Center Universitas Gunadarma senilai Rp 750.000,-.';
    ...
    function createregisterpengajar() {
        if ($this->input->post('submit')){
            $id_pengajar	    = $this->input->post('id_pengajar');
			$email	    		= $this->input->post('email');
            $term       		= $this->input->post('term');
			
			$contentemail	= 'Terima Kasih anda telah terdaftar sebagai pengajar kursus brevet yang diselenggarakan oleh Tax Center Universitas Gunadarma.';
			
			if(!empty($npm)){
					$checkuser = "SELECT * FROM data_pengajar WHERE `id_pengajar`='".$id_pengajar."'";
					$query=$this->db->query($checkuser);
						if($query->num_rows() > 0){
							$this->session->set_flashdata('message', '<p style="text-align : center;">Anda sudah terdaftar. Silahkan Login</p>');
							redirect('index/login');
						}else{
							$this->m_login->create('data_pengajar',$this->field_datauserpengajar());
							$this->m_login->create('user',$this->field_datauserpengajar2());
							$this->email->to($email);
							$this->email->from('noreply@taxcenter.gunadarma.ac.id','No Reply Tax Center UG');
							$this->email->Subject('Registrasi Pengajar Kursus Brevet');
							$this->email->message($contentemail);
							$this->email->send();
							redirect('index/complete_pengajar', 'refresh');
						}
			}else{
				$this->session->set_flashdata('message', '<p style="text-align : center;">Silahkan isi form dibawah ini terlebih dahulu.</p>');
				redirect('index/pengajar');
			}
        }else{
            redirect('index/pengajar', 'refresh');
        }
    }
	
	function complete_pengajar() {
		$this->load->helper(array('form'));
		$isi['kelassingle'] = 'registrasi';$isi['kelasroot']   = ''; $isi['kelas']     = '';
        $isi['id']          =$this->m_login->getkodeunik('data_pengajar','id');
        $isi['kursus']      =$this->db->get('data_kursus');
        $isi['judul']       = 'Registrasi';
        $isi['content']     ='frontend/registrasi-finish-pengajar';
        $this->load->view('frontend/index',$isi);
    }
    
    function field_datauserpengajar()
    {
        return array(
            'id'            => $this->input->post('id'),
            'nama_lengkap'  => $this->input->post('nama_lengkap'),
	...
	function field_datauserpengajar2()
    {
        $password   = $this->input->post('password');
        $npassword  = $this->encryptIt( $password );
        return array(
