...
class home extends CI_Controller {
    ...
    //Jadwal=====================================================================================
    private $fieldpkjadwal    = 'id';
    private $tablejadwal      = 'tr_jadwal';
    private $navjadwal        = 'Jadwal';
    private $juduljadwal      = 'Jadwal';
    private $folderjadwal     = 'dsis_jadwal';
    private $linkjadwal       = 'jadwal';
	
    public function jadwal()
    {
        $isi['jadwal']       =True;
        $isi['nav']         =$this->navjadwal;
        $isi['judul']       =$this->juduljadwal;
        $isi['content']     =$this->folderjadwal.'/'.$this->read;
        $idgelombang		=$this->db->where('id_mahasiswa',$this->session->userdata('id'))->get('tr_mahasiswa_kursus');
		$row2 				=$idgelombang->row();
		$isi['query']		=$this->db
								  ->select('p.id,p.jam,p.tanggal,p.keterangan,d.gelombang,d.kelas_gelombang,p.id_kursus,e.nama_depan,e.nama_belakang,f.nama_aspek,c.nama_kursus')
    ...
    public function registrasi()
    {
        ...
        $isi['query']           =$this->db
                                    ->select('p.id,p.status_pembayaran,p.tgl_register,c.nama_kursus,d.npm,d.kelas,d.domisili,d.nama_lengkap,d.nama_depan,d.nama_belakang,d.no_hp,e.gelombang,e.kelas_gelombang,f.nama_ujian')
                                    ->join('data_kursus c','p.id_kursus = c.id','LEFT')
                                    ...
                                    ->order_by('e.gelombang','DESC')
                                    ->order_by('p.status_pembayaran','DESC')
                                    ->order_by('p.tgl_register','DESC')
    ...
    public function createpembayaran()
    {
        ...
        $isi['kursus']      =$this->db->get($this->tablekursus);
        $isi['mahasiswa']   =$this->db->order_by('nama_lengkap','ASC')->get($this->tablemahasiswa);
        $isi['aksi']        ='create';
	...
	public function exportsoal($id)
    {
        $isi['query']       =$this->db
                                  ...
                                  ->where('p.id_tr_ujian', $id)
								  ->get($this->tablesoal.' p');
        $this->load->view('dst_soal/readexport',$isi);
    }
    ...
    public function up()
    {
        ...
        $isi['query']       =$this->db
								  ->select('p.id, p.email, p.password , c.nama_lengkap, c.nama_depan, c.nama_belakang, c.kota')
								  ->join('data_register c','p.id_tabel = c.id','LEFT')
								  ->get($this->tableuser.' p');
        $isi['linkexport']  =$this->home.'/export_up/';
        $this->load->view('index',$isi);
    }
    ...
    public function export_up()
    {
        $isi['query']       =$this->db
								  ...
								  ->join('data_register c','p.id_tabel = c.id','LEFT')
								  ->get($this->tableuser.' p');
        $this->load->view('dst_up/read2',$isi);
    }
    ...
    private $judulaspekpenilaian      = 'Materi';
    ...
    public function aspekpenilaian()
    {
        ...
        $isi['query']       =$this->db
								  ->select('p.id,c.nama_kursus,p.nama_aspek')
								  ->join('data_kursus c','p.id_kursus = c.id','LEFT')
    ...
    public function deletehasil_ujian($id_ujian,$pk)
    {
		...
		$this->db->query("DELETE FROM tr_jawab WHERE id_tes = ".$id_ujian." AND id_user = ".$id_user);
        $this->session->set_flashdata('message','<div class="panel-body"><div class="alert alert-success">Anda Telah Berhasil Melakukan Perubahan Data.</div></div>');
        redirect($this->home.'/'.$this->linkhasil_ujian);
    }
    ...
    public function rombelgelombang($id)
    {
        ...
        $isi['query']       =$this->db
                                  ->select('p.id_mahasiswa, p.status_pembayaran, d.npm, d.no_hp, d.kelas, d.domisili, d.nama_lengkap, d.nama_depan, d.nama_belakang, c.gelombang, c.kelas_gelombang')
                                  ->join('data_register d','p.id_mahasiswa = d.id','LEFT')
                                  ->join('data_gelombang c','p.id_gelombang = c.id','LEFT')
                                  ->where('p.id_gelombang', $id)
								  ->order_by('status_pembayaran', 'ASC')
        ...
        $isi['linkexport']  =$this->home.'/export_rombelgelombang/'.$id;
        $this->load->view('index',$isi);
    }
    ...
    public function gelombang()
    {
        ...
        $isi['query']       =$this->db
                                  ->select('p.id,p.gelombang,p.kelas_gelombang,p.pj,p.id_kursus,c.nama_kursus')
								  ->join('data_kursus c','p.id_kursus = c.id','LEFT')
								  ->get($this->tablegelombang.' p');
        ...
        $isi['linknilai']   =$this->home.'/nilaigelombang/';
        $isi['linkjadwal']  =$this->home.'/jadwalgelombang/';
    ...
    private function field_datagelombang()
    {
        return array(
            ...
            'kelas_gelombang'             => $this->input->post('kelas'),
            'pj'                => $this->input->post('pj')
            );
    }

    public function export_rombelgelombang($id)
    {
        $isi['gelombang']   =True;
        $isi['query']       =$this->db
                                  ->select('p.id_mahasiswa, p.status_pembayaran, d.npm, d.kelas, d.nama_lengkap, d.nama_depan, d.nama_belakang, c.gelombang, c.kelas_gelombang')
                                  ->join('data_register d','p.id_mahasiswa = d.id','LEFT')
                                  ->join('data_gelombang c','p.id_gelombang = c.id','LEFT')
                                  ->where('p.id_gelombang', $id)
								  ->order_by('nama_depan, nama_lengkap', 'ASC')
                                  ->get('tr_mahasiswa_kursus p');
        $this->load->view('dst_gelombang/rombelgelombangexport',$isi);
    }
	
	//Jadwal Gelombang=====================================================================================
    private $fieldpkjadwalgelombang    = 'id';
    private $tablejadwalgelombang      = 'tr_jadwal';
    private $navjadwalgelombang        = 'Jadwal';
    private $juduljadwalgelombang      = 'Jadwal';
    private $folderjadwalgelombang     = 'dst_gelombang';
    private $linkjadwalgelombang       = 'jadwalgelombang';
	
	public function jadwalgelombang($id)
    {
        $isi['gelombang']       =True;
        $isi['nav']         =$this->navjadwalgelombang;
        $isi['judul']       =$this->juduljadwalgelombang;
        $isi['content']     =$this->folderjadwalgelombang.'/'.$this->read.'jadwalgelombang';
        $isi['query']       =$this->db
                                  ->select('p.id,p.jam,p.tanggal,p.keterangan,d.gelombang,d.kelas_gelombang,p.id_kursus,e.nama_depan,e.nama_belakang,f.nama_aspek,c.nama_kursus')
								  ->join('data_kursus c','p.id_kursus = c.id','LEFT')
								  ->join('data_gelombang d','p.id_gelombang = d.id','LEFT')
								  ...
								  ->get($this->tablejadwalgelombang.' p');
        $isi['link2']       =$this->home.'/'.$this->linkgelombang;
        $isi['linkcreate']  =$this->home.'/'.$this->create.$this->linkjadwalgelombang.'/'.$id.'/';
        $isi['linkupdate']  =$this->home.'/'.$this->update.$this->linkjadwalgelombang.'/'.$id.'/';
        $isi['linkdelete']  =$this->home.'/'.$this->delete.$this->linkjadwalgelombang.'/';
        $this->load->view('index',$isi);
    }
	
    public function createjadwalgelombang($id)
    {
        if ($this->input->post('simpan')){
            $this->m_global->create($this->tablejadwalgelombang,$this->field_datajadwalgelombang());
            $this->session->set_flashdata('message','<div class="panel-body"><div class="alert alert-success">Anda Telah Berhasil Melakukan Perubahan Data.</div></div>');
            redirect($this->home.'/'.$this->linkjadwalgelombang.'/'.$id);
        }else{
        $isi['gelombang']       =True;
        $isi['nav']         =$this->navjadwalgelombang.' | Create';
        $isi['judul']       ='Create '.$this->juduljadwalgelombang;
        $isi['content']     =$this->folderjadwalgelombang.'/formjadwalgelombang';
        $isi['link2']       =$this->home.'/'.$this->linkjadwalgelombang.'/'.$id;
        $isi['link']        =$this->home.'/'.$this->create.$this->linkjadwalgelombang;
        $isi['kursus']      =$this->db->get($this->tablekursus);
        $isi['pengajar']    =$this->db->get($this->tablepengajar);
        $isi['materi']      =$this->db->get($this->tableaspekpenilaian);
		$isi['gelombang']   =$this->db
                                  ->where('id', $id)
                                  ->get($this->tablegelombang);
        $isi['id_gelombang']=$id;
        $isi['aksi']        ='create';
        $isi['id']          =$this->m_global->getkodeunik($this->tablejadwalgelombang,$this->fieldpkjadwalgelombang);
        $this->load->view('index',$isi);
        }
    }

    public function updatejadwalgelombang($id,$pk)
    {
        if ($this->input->post('simpan')){
            $this->m_global->update($this->tablejadwalgelombang,$this->fieldpkjadwalgelombang,$pk,$this->field_datajadwalgelombang());
            $this->session->set_flashdata('message','<div class="panel-body"><div class="alert alert-success">Anda Telah Berhasil Melakukan Perubahan Data.</div></div>');
            redirect($this->home.'/'.$this->linkjadwalgelombang.'/'.$id);
        }else{
        $isi['gelombang']       =True;
        $isi['nav']         =$this->navjadwalgelombang.' | Update';
        $isi['judul']       ='Edit '.$this->juduljadwalgelombang;
        $isi['content']     =$this->folderjadwalgelombang.'/formjadwalgelombang';
        $isi['query']       =$this->m_global->read_update($this->tablejadwalgelombang,$this->fieldpkjadwalgelombang,$pk);
        $isi['link2']       =$this->home.'/'.$this->linkjadwalgelombang.'/'.$id;
        $isi['link']        =$this->home.'/'.$this->update.$this->linkjadwalgelombang.'/';
        $isi['kursus']      =$this->db->get($this->tablekursus);
        $isi['pengajar']    =$this->db->get($this->tablepengajar);
        $isi['materi']      =$this->db->get($this->tableaspekpenilaian);
		$isi['gelombang']   =$this->db
    ...
    public function deletejadwalgelombang($pk)
    {
        $this->m_global->delete($this->tablejadwalgelombang,$this->fieldpkjadwalgelombang,$pk);
        $this->session->set_flashdata('message','<div class="panel-body"><div class="alert alert-success">Anda Telah Berhasil Melakukan Perubahan Data.</div></div>');
        redirect($this->home.'/'.$this->linkjadwalgelombang);
    }
    
    private function field_datajadwalgelombang()
    {
        return array(
            'id'                => $this->input->post('id'),
			'id_kursus'			=> $this->input->post('id_kursus'),
